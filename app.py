...
# ===== 4. محرك التنبيهات الخلفي المستمر والأهداف الصارمة =====
...
def alert_processing_loop():
    """الرادار الخلفي الذي يفحص الأسعار الفورية ويرسل التنبيهات بناءً على أرقام دقيقة"""
    while True:
        try:
            ...
            if price_21 > 0:
                ...
                for user in users:
                    ...
                    # فحص شرط كسر السعر الأعلى (جني الأرباح) بالملي
                    if high and float(price_21) >= float(high) and last_high != today_str:
                    ...
                    # فحص شرط كسر السعر الأدنى (دعم الشراء) بالملي
                    if low and float(price_21) <= float(low) and last_low != today_str:
...
# تصحيح الخاصية لتكون unsafe_allow_html
st.markdown("""
    <style>
...
# --- القسم الأول: شاشات الأسعار اللحظية مع تصحيح صياغة الـ HTML بالكامل ---
...
with col3:
    st.markdown(f"<div class='price-card'><h4 style='color:#00d4ff;'>🏅 الأوقية محلياً</h4><h2>{float(usd_price * usd_egp):,.2f} ج.م</h2></div>", unsafe_allow_html=True)
...
with reg_col2:
    target_high = st.number_input("🚀 نبهني لو عيار 21 رفع وكسر السعر ده (جني أرباح):", min_value=0.0, step=10.0, value=st.session_state["target_high"])
    target_low = st.number_input("🔻 نبهني لو عيار 21 نزل وكسر السعر ده (دعم شراء):", min_value=0.0, step=10.0, value=st.session_state["target_low"])
...
if st.button("💾 تفعيل الاشتراك وحفظ الإعدادات"):
    if u_name and u_tg:
        ...
        if success:
            ...
            st.session_state["target_high"] = target_high
            st.session_state["target_low"] = target_low
